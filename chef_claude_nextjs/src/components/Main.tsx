    ...
    <div className="mt-16 flex flex-col items-center align-center p-5">
      <div className="flex flex-col">
        <form className="flex flex-row gap-3" action={addIngredient}>
          ...
          <button onClick={handleReset} className="bg-white text-black border-2 hover:bg-gray-200 transition-all duration-500 py-2 px-8 rounded-lg">
            Reset
          </button>
        </form>

        {ingredients.length > 0 ? <section className="m-5">
          ...
          {ingredients.length > 2 ? <div className="mt-12">
            <CallToAction />
          </div> : null}            
        </section> : null}
      </div>
